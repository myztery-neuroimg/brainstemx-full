...
- **Atlas registry** (`atlas_registry.sh`, sourced by `multi_atlas.sh`) — any further MNI-space atlas is a config entry (`MULTI_ATLAS_EXTRA` + `ATLAS_<KEY>_*`): label images, 4D probability atlases, per-nucleus probability directories or single probability maps are normalised to a cached MNI dseg + LUT (FSL XML / FreeSurfer LUT / text, with the Label-vs-Probabilistic offset rule), brought into FSL MNI152 (NLin6Asym) space (TemplateFlow 2009c→NLin6 transform or an explicit `ATLAS_<KEY>_XFM` chain; skipped, never mis-warped, when no transform exists), optionally restricted to the Harvard-Oxford brainstem extent (mandatory for whole-brain tract atlases), then warped with the **same shared SyN transform** and split into `<key>_<name>_label<v>.nii.gz` masks. Shipped entries: JHU ICBM-DTI-81 pontine tracts and XTRACT (both ship with FSL), Harvard AAN v2.0, locus-coeruleus and dorsal-raphe maps, and the NextBrain MNI152 rendering. `MULTI_ATLAS_SOURCE_TAGS` is the single list every downstream consumer (analysis discovery/provenance, reporting, visualisation, viewer) iterates over. No atlas data is vendored.
- **NextBrain histological-atlas nuclei** (`brainstem_nextbrain.sh`) — wraps FreeSurfer 8's `mri_histo_atlas_segment_fireants` (Casamitjana et al., Nature 2025): Bayesian, contrast-agnostic, no recon-all, ~15–30 min per side on CPU; brainstem labels selected by name regex from `lut.txt` land as `nextbrain_<name>_<l|r>_label<v>.nii.gz`. One more gated, non-fatal parallel path in `all` mode.
- **Posterior detection engine** (`lesion_posterior.py`, `DETECTION_ENGINE=posterior`) — per region: lesion-uncontaminated robust null (biweight + MAD, spatial trend, CSF/PV gating), Efron two-groups empirical null with local false-discovery rates, posterior-expected-FDR decision (no SD multipliers, no percentile floor; a healthy region yields nothing), mean-field MRF prior, cluster filter, calibrated posterior maps, reliability flags. Legacy GMM chain kept as `DETECTION_ENGINE=legacy`. Phantom A/B: brainstem Dice 0.01 → 0.85, WM 0.00 → 0.89 (`docs/detection_engine.md`, `docs/benchmarking.md`).
- **Consensus vote families** — with many correlated atlas sources the cross-source agreement count votes per source family (`CONSENSUS_VOTE_BY=family`: atlas / freesurfer / harvard_oxford / synthseg) so a "≥ 2 sources" consensus reflects independent evidence, not the same prior warped twice; regions too small for a mixture fit (`ANALYSIS_MIN_REGION_VOXELS`) are recorded in `region_skips.tsv`.
- **Visual QC renderer** (`viz_render.py` + `visualization.sh::viz_render/viz_figure/viz_gallery`) — all QC figures are drawn headlessly in python (numpy/nibabel/matplotlib): mask/label/continuous-map mosaics centred on the structure of interest with L/R annotation and consistent per-source colours, registration checkerboards with moving-image edges, before/after difference panels, mixture-fit histograms, and a per-stage gallery (`visualizations/index.html`). FSL `slicer` is only a fallback.
...
- **JHU ICBM-DTI-81** — Mori et al., MRI Atlas of Human White Matter, Elsevier 2005; Hua et al., NeuroImage 2008;39(1):336-347
- **XTRACT** — Warrington et al., NeuroImage 2020;217:116923
- **Harvard AAN atlas v2.0** — Edlow et al., Sci Transl Med 2024;16(745):eadj4303 (Dryad doi:10.5061/dryad.zw3r228d2)
- **Locus coeruleus atlases** — Ye et al., NeuroImage 2021;225:117487 (7T, NITRC); Dahl et al. 2022 LC meta-mask (OSF sf2ky)
- **Dorsal raphe mask** — Wearn et al., Zenodo 2024, doi:10.5281/zenodo.10680563
- **NextBrain** — Casamitjana et al., Nature 2025; Puonti et al., Imaging Neuroscience 2026; MNI152 rendering github.com/compneurobilbao/nextbrain-mni-atlas
- **TemplateFlow** — Ciric et al., Nat Methods 2022;19:1568-1571
