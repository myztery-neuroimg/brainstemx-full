...
# Function to optimize dcm2niix flags for Siemens DICOM files
# This addresses the issue where dcm2niix marks all images as duplicates
import_optimize_conversion_flags() {
    local dicom_dir="$1"
    ...
    if [ ${#dicom_files[@]} -gt 0 ]; then
        ...
        log_message "Found sample DICOM file for vendor detection: $sample_dicom"
    else
    ...
    if [ -z "$sample_dicom" ] || [ ! -f "$sample_dicom" ]; then
        log_formatted "WARNING" "No DICOM files found for vendor detection"
        return 0
    fi
    
    # Detect vendor using dcmdump if available
    local vendor="UNKNOWN"
    if command -v dcmdump &>/dev/null; then
        local manufacturer=$(dcmdump "$sample_dicom" 2>/dev/null | grep -i "Manufacturer" | head -1)
        
        if [[ "$manufacturer" == *"SIEMENS"* ]]; then
            vendor="SIEMENS"
        elif [[ "$manufacturer" == *"PHILIPS"* ]]; then
            vendor="PHILIPS"
        elif [[ "$manufacturer" == *"GE"* ]]; then
            vendor="GE"
        fi
    fi
    
    # Set optimal flags based on vendor
    case "$vendor" in
        "SIEMENS")
            log_message "Detected Siemens DICOM - using optimized conversion flags"
            export DICOM_VENDOR="SIEMENS"
            # These flags help prevent the "duplicate detection" issue in Siemens files
            export DICOM_VENDOR_FLAGS="--no-collapse --ignore-derived --exact_values 1 -m y -i y"
            ;;
        "PHILIPS")
            log_message "Detected Philips DICOM - using appropriate conversion flags"
            export DICOM_VENDOR="PHILIPS"
            export DICOM_VENDOR_FLAGS="--exact_values 1 -m y"
            ;;
        "GE")
            log_message "Detected GE DICOM - using appropriate conversion flags"
            export DICOM_VENDOR="GE"
            export DICOM_VENDOR_FLAGS="-m y"
            ;;
        *)
            log_message "Unknown vendor - using safe default conversion flags"
            export DICOM_VENDOR="UNKNOWN"
            export DICOM_VENDOR_FLAGS="--exact_values 1 -m y -i y"
            ;;
    esac
